...
from momarl.mo_predpreygrass.predpreygrass.environments.mo_predpreygrass import raw_env
...
for i in range(num_episodes):
    ...
    for agent in raw_env.agent_iter():
        ...
        if termination or truncation:
            ...
            reward = raw_env.reward_spaces[agent].sample()
            print(f"Agent {agent} takes action {action} and receives reward {reward}")
            """
            0: [-1, 0], # move left
            1: [0, -1], # move up
            2: [0, 0], # stay
