...
if __name__ == "__main__":
    ...
    # ---------------------- #
    mkdir_p("momarl/beach/iql/results/runs")
    if args.random:
        df_total.to_csv(f"momarl/beach/iql/results/runs/BPD_{args.num_agents}_random.csv", index=False)
    else:
        df_total.to_csv(
            f"momarl/beach/iql/results/runs/BPD_{args.num_agents}_{args.reward_scheme}.csv", index=False
        )

    # ------------------------------ #
    # Saving Non-Dominated Solutions #
    # ------------------------------ #
    mkdir_p("momarl/beach/iql/results/nds")
    ...
    # Save to csv
    if args.random:
        df.to_csv(f"momarl/beach/iql/results/nds/BPD_{args.num_agents}_random.csv", index=False)
    else:
        df.to_csv(f"momarl/beach/iql/results/nds/BPD_{args.num_agents}_{args.reward_scheme}.csv", index=False)
